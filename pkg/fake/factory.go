...
func (f *Factory) Config() (*config.HookConfig, error) {
	hookconfig := config.LoadConfig(filepath.Join(f.rootDir), "config")
	return &hookconfig, nil
}

func (f *Factory) ConfigFileName(filename string) (*config.HookConfig, error) {
	hookconfig := config.LoadConfig(filepath.Join(f.rootDir), filename)
	return &hookconfig, nil
}
